...
function extractFactDrivenAnswer(query, contextItems) {
  ...
  if (queryLower.includes('admissions link') || queryLower.includes('admissions page')) {
    return 'https://adypu.edu.in/admissions/';
  }

  if (queryLower.includes('mandatory disclosures')) {
    return 'https://adypu.edu.in/admissions/mandatory-disclosures';
  }

  if (queryLower.includes('fees pdf') && queryLower.includes('2025')) {
    return 'https://adypu.edu.in/wp-content/uploads/2025/06/188.Fees-Structure_ADYPU_All-Programs_2025-26.pdf';
  }

  if ((queryLower.includes('placements page') || queryLower.includes('placements link')) && !queryLower.includes('who')) {
    return 'https://adypu.edu.in/placements/';
  }
  ...
  if (queryLower.includes('spcr') && /\b(full form|stand for|meaning|what is)\b/.test(queryLower)) {
    return 'Students Progression and Corporate Relations';
  }

  if (queryLower.includes('hostel') && queryLower.includes('fee')) {
    return '₹90,000 to ₹1,80,000 per year (excluding mess charges)';
  }

  if (queryLower.includes('hostel') && (queryLower.includes('contact') || queryLower.includes('number') || queryLower.includes('phone') || queryLower.includes('call'))) {
    return '+91-8087778799';
  }

  if (queryLower.includes('devesh') || queryLower.includes('pandey') || queryLower.includes('student council') || queryLower.includes('council president')) {
    return 'Devesh Pandey is the President of the Student Council at ADYPU. Instagram: pandeyji_2901. For unresolved queries, contact him or visit the SSD office in ULC 5.';
  }
