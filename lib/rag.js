...
export async function generateAnswer({ query, language, history, contextItems, responseStyle = 'standard', clientApiKey }) {
  ...
  if (!apiKey) {
    return {
      answer: offlineAnswer || '__FALLBACK__',
      usedModel: 'offline-extractive'
    };
  }
  ...
  try {
    ...
    return {
      answer: response.choices?.[0]?.message?.content?.trim() || offlineAnswer || '__FALLBACK__',
